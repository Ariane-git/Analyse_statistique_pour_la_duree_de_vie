    ...
    if charger_rsf or st.session_state.get('rsf_loaded'):
        ...
        if rsf_pkg:
            ...
            if 'permutation_importance' in rsf_pkg:
                _feats_imp = rsf_pkg.get('features', FEATURES)
                imp = pd.Series(rsf_pkg['permutation_importance'], index=_feats_imp).sort_values(ascending=False)
...
# ============================================================================
    ...
    rsf_mod = rsf_pkg['model']
    # IMPORTANT : toujours utiliser les features du pkl, pas la constante FEATURES,
    # pour éviter un mismatch si le modèle a été entraîné avec une autre liste de colonnes.
    feats_rsf = rsf_pkg.get('features', FEATURES)
    ...
    valeurs_saisies = {
        ...
        'groupe_age'             : ga,
        'statut_matrimonial'     : sm,
        'niveau_instruction'     : instr,
        'quintile_richesse'      : quint,
        'milieu_residence'       : milieu,
        'region'                 : region,
        'religion_code'          : relig,
        'ecoute_radio'           : radio,
        'vision_television'      : tv,
        'lecture_journaux'       : jrnx,
        'nombre_enfants_vivants' : nb_enf,
        'nombre_naissances'      : nb_naiss,
        'occupation'             : occ,
        'type_union'             : typ_union,
    }

    # On ne garde / n'ordonne que les colonnes que le modèle chargé connaît réellement.
    # Cela évite tout ValueError de feature names si le pkl a été entraîné avec une
    # liste de variables différente de la constante FEATURES ci-dessus.
    colonnes_manquantes = [f for f in feats_rsf if f not in valeurs_saisies]
    if colonnes_manquantes:
        st.error(
            "❌ Le modèle RSF chargé attend des variables non saisissables dans ce "
            f"formulaire : {colonnes_manquantes}. Le fichier rsf_model.pkl ne correspond "
            "probablement pas à la version actuelle de app.py — régénérez-le avec le "
            "notebook ml_survie_contraception à jour."
        )
        st.stop()

    profil_saisie = pd.DataFrame([[valeurs_saisies[f] for f in feats_rsf]], columns=feats_rsf)
